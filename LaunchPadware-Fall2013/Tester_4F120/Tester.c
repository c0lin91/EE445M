// Tester.c
// Runs on LM4F120
// Interface with a custom Booster Pack module to test a LM4F120
// LaunchPad.  After the program is downloaded and the Booster Pack
// is attached, the LM4F120 will perform a self-diagnostic to check
// for the most common hardware failures and output the results to
...
// 10k and 1k resistors such that no two pins have less than 2k of resistance
// between them (NOTE: except for PB7 and PD1 and PB6 and BD0, which are
// connected together by default with zero-ohm R9 and R10 on the LaunchPad.)

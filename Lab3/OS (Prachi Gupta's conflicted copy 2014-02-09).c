...
int OS_AddThread (void (*threadName) (void), int threadId, int sleepState, int priority)  {
	...
	SetInitialStack(threadId); Stacks[threadId][STACKSIZE-1] = (long)threadName;   // initialize stack and pc on the stack
	tcbs[threadId].prevThread = &tcbs[(threadId-1)%NUMTHREADS]; 
	tcbs[threadId].nextThread = &tcbs[(threadId+1)%NUMTHREADS]; 

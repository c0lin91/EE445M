...
void PortF_Init(void){ unsigned long volatile delay;
  ...
  GPIO_PORTF_DIR_R |= 0x0E;    // (c) make PF1-2 out
  GPIO_PORTF_AFSEL_R &= ~0x0E;  //     disable alt funct on PF1-2
  GPIO_PORTF_DEN_R |= 0x0E;     //     enable digital I/O on PF1-2   
...
unsigned long OS_Time(void){
	SysTime = TIMER_TAMATCHR_TAMR_M - TIMER3_TAMR_R;
	return SysTime; 
} //need to fix this to ns
...
void SysClock_Init(int period){
			...
			TIMER3_TAILR_R = TIMER_TAMATCHR_TAMR_M;         // 4) reload value
